...
# ╔═╡ 89030793-d98f-451e-85e8-3efdd991641c
using Plots, PlutoUI,ControlSystems,LaTeXStrings;

# ╔═╡ 8bb82f5f-db4f-4c30-9ca6-4818643f9b54
T = 1e-3;

# ╔═╡ 68c2477d-e8ee-4f9a-9763-89d86045344a
@bind τ Select([1e-3, 100e-6, 30e-6])

# ╔═╡ d8d17b87-b52c-4fb0-98bb-643c795f2b16
ω_1 = 2*pi/T;

# ╔═╡ 2a228237-d95b-4d23-8495-318de85f0828
ts=(-1:T:3)

# ╔═╡ ef3c4f0e-aa24-4438-9ce1-461f95d7f7d5
rect_periodic(t) = abs(mod(t+T/2,T)-T/2) < T/4 ? 1 : 0

# ╔═╡ 6c7cd479-7e59-4e34-841a-9354787cdf06
plot(ts*T,rect_periodic.(ts*T),label="")

# ╔═╡ 257c93ad-4c9d-410a-9901-856d478e4705
c_x(k) = 1/2 .* sinc.(k/2) # da ist schon *pi drin

# ╔═╡ 8bef6d81-9d5f-4092-aa76-2f9e8bdbbc08
lim = 15

# ╔═╡ cc75ef21-ee2c-49fb-a3fa-327f7666ffba
ks = -lim:lim

# ╔═╡ fe581211-b30c-46ad-b6d1-e6cd0b251922
c_x(ks)

# ╔═╡ 7b051054-8be5-4311-8cc9-26fcea71fe44
begin
	p1 = plot(ks,c_x(ks),line=:stem,marker=:o, markercolor=:white,title=L"\textrm{Komplex: } c_x")
	p2 = plot(ks,abs.(c_x(ks)),line=:stem,marker=:o, markercolor=:white,title=L"\textrm{Betrag: } c_x")
	p3 = plot(ks,angle.(c_x(ks)),line=:stem,marker=:o, markercolor=:white,title=L"\textrm{Phase: } c_x")

	plot(p1, p2, p3, layout=(3,1),legend=false)

end

# ╔═╡ 1b072d3b-76b5-409b-98be-197a40fe0633
sum(c_x(ks))

# ╔═╡ 4c8730b3-15c1-4b96-82fd-0ea8ffd39888
H = tf(1,[τ,1])

# ╔═╡ f3900feb-7d8a-4145-bb88-35d0b4bb56e4
F = freqrespv(H,ω_1*ks)

# ╔═╡ 3add2062-3cba-4a3b-8a2e-38bc32fa55d7
begin
	
	p4 = plot(ks,abs.(freqrespv(H,ω_1*ks)),marker=:o, markercolor=:white,title="Amplitudengang TP")
	p5= plot(ks,angle.(freqrespv(H,ω_1*ks)),marker=:o, markercolor=:white,title="Phasengang TP")

	plot(p4, p5, layout=(2,1),legend=false)

end

# ╔═╡ 1e1ec204-0a9e-4d15-94a5-c3b3911c4627
c_y(k) = freqrespv(H,ω_1*k).*c_x(k)

# ╔═╡ 335d2f74-1c0d-4aa9-ae70-00ffbb39b796
begin

	p6 = plot(ks,abs.(c_y(ks)),line=:stem,marker=:o, markercolor=:white,title=L"\textrm{Betrag: } c_y")
	p7 = plot(ks,angle.(c_y(ks)),line=:stem,marker=:o, markercolor=:white,title=L"\textrm{Phase: } c_y")

	plot(p6, p7, layout=(2,1),legend=false)

end

# ╔═╡ 23d9344f-6a54-4325-89ec-f4c22fdd8a97
# ╠═╡ disabled = true
#=╠═╡
A = [c_x(ks) freqrespv(H,ω_1*ks) c_y(ks)]
  ╠═╡ =#

# ╔═╡ a5f4c083-a1b1-43f1-93f3-5e0054e51cf7
# ╠═╡ disabled = true
#=╠═╡
A[1]
  ╠═╡ =#

# ╔═╡ ef3c1a8f-0a9b-48ad-a96d-711561aec032
y(t) = sum(c_y(ks).*exp.(im*ks*ω_1*t))

# ╔═╡ f9378a55-9130-4d93-b6ca-480cf9af45f1


# ╔═╡ 060c3987-3ccd-4323-b7db-40725703480f
begin
	plot(ts*T,real.(y.(ts*T)),label="")
	plot!(ts*T,rect_periodic.(ts*T),label="")
end
...
manifest_format = "2.0"
project_hash = "26874a5a8cce4de17b127d30995263b102ac9c90"
